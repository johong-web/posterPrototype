...
<!-- Added by: deim, at: Thu  3 Dec 2020 12:31:21 GMT -->
...
<https://libs.zappar.com/zappar-aframe/3.2.0/zappar-aframe.zip>
...
```html
<script src="https://libs.zappar.com/zappar-aframe/3.2.0/zappar-aframe.js"></script>
```
...
_Functional but not our primary support target (support quality):_
...
\* Browsers without motion sensor access (e.g desktop browsers) don't support world tracking (including instant world tracking) or attitude-based camera poses.
...
```html
<a-camera zappar-camera id="zappar-camera"> </a-camera>
...
```html
<a-scene xr-mode-ui="enabled: false" zappar-environment-map> </a-scene>
```
...
- _World Tracking_ lets you position 3D content at a point in space in front of the user, such as on a surface. With this tracking type you could build a 3D model viewer that lets users walk around to view the model from different angles, or an experience that places an animated character in their room. Content has approximately accurate scale, with 1 unit of 3D space being approximately 1 meter in physical space.
- _Instant World Tracking_, as with world tracking, lets you position 3D content at a point in space in front of the user. Unlike world tracking (above), instant tracking does not require the user to move their device during an initialization phase, however the scale of the content is relative and the anchor point may drift if the device moves quickly or the view is obstructed.
...
```html
<a-entity
	geometry="primitive: face-mesh; face: #my-face-tracker; model: full-head-simplified; fill-mouth: true; fill-eye-left: true; fill-eye-right: true; fill-neck: true;"
	material="src:#my-texture; transparent: true;"
></a-entity>
```
...
The `zappar-user-placement` component allows tracking content to a surface in front of the user, at approximately correct scale. The component provides UI that allows the user to adjust the location of the anchor point in space if they wish to.

You can use the component like this:

```html

<a-entity zappar-user-placement="">
	<!-- PLACE CONTENT TO APPEAR IN THE WORLD HERE -->
</a-entity>

```

A number of options can be passed into the component:

| Option | Default | Description |
| - | - | - |
| `placeOnPlanes` | `true` | content will be placed onto a flat surface (a plane) in the user's environment
| `placeOnGround` | `false` | content will only be placed on a plane that's likely to be the ground in front of the user
| `visualizePlanesDuringPlacement` | `true` | display a visualization of the planes in the environment during placement mode
| `showInitializationUI` | `true` | display an animation while the world tracker is initializing
| `showPlaceButton` | `true` | during placement mode, shows a button for the user to confirm their chosen location
| `showRestartPlacementButton` | `true` | shows a button that allows the user to choose a new location in the world for the content


### Instant World Tracking

Instant tracking allows content to be positioned at a point in space in front of the user. Unlike world tracking (above), instant tracking does not require the user to move their device during an initialization phase, however the scale of the content is relative and the anchor point may drift if the device moves quickly or the view is obstructed.
...
```html
<a-entity zappar-image="target: #target-file; enabled: false" id="anchor"> </a-entity>
```
